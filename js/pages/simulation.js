...
function runSimulation() {
    ...
    resultArea.innerHTML = `
        <div class="sim-current-score">
            現在の区分: <span style="color: ${currentCategory.color}; font-weight: bold;">${currentCategory.text}</span><br>
            現在のビッグクラブスコア: <strong>${currentScore.toFixed(2)}</strong>
        </div>
        <div class="sim-new-score">
            予測区分: <span style="color: ${newCategory.color}; font-weight: bold;">${newCategory.text}</span><br>
            予測スコア <span class="arrow">→</span> <span class="score-value">${newScore.toFixed(2)}</span>
        </div>
    `;
}
...
function displayClubData(clubName) {
    ...
    resultArea.innerHTML = `
        ...
        <div class="sim-new-score">
            <p style="margin:0; font-size:0.8em; color:#ccc;">数値を変更してください</p>
        </div>
    `;
}
...
function populateClubSelect(league) {
    ...
    const clubData = getClubData();
    const clubsInLeague = clubData.filter(c => c.p === league).sort((a, b) => a.name.localeCompare(b.name, 'ja'));
...
export default function initSimulationPage() {
    const leagueSelect = document.getElementById('sim-league-select');
    if (!leagueSelect) {
        console.error("シミュレーションページの要素が見つかりませんでした。");
        return;
    }

    if (!leagueSelect.dataset.initialized) {
        const clubSelect = document.getElementById('sim-club-select');
        const inputs = document.querySelectorAll('.sim-input');
        ...
        leagueSelect.dataset.initialized = 'true';
    }
