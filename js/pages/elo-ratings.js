...
const abbreviationMap = {
    ...
    '清水エスパルス': '清水', '横浜ＦＣ': '横浜FC', 'Ｖ・ファーレン長崎': '長崎', 'ファジアーノ岡山': '岡山', 'ベガルタ仙台': '仙台', 'ジェフユナイテッド千葉': '千葉', 'いわきＦＣ': 'いわき', '徳島ヴォルティス': '徳島', 'ヴァンフォーレ甲府': '甲府', 'モンテディオ山形': '山形', '愛媛ＦＣ': '愛媛', 'ブラウブリッツ秋田': '秋田', '大分トリニータ': '大分', 'レノファ山口ＦＣ': '山口', '鹿児島ユナイテッドFC': '鹿児島', '水戸ホーリーホック': '水戸', 'ロアッソ熊本': '熊本', '藤枝ＭＹＦＣ': '藤枝', '栃木ＳＣ': '栃木', 'ザスパ群馬': '群馬', 'ＲＢ大宮アルディージャ': '大宮', 'ＦＣ今治': '今治',
...
function createRatingTableHTML(league, data) {
    ...
    const rowsHTML = sortedTeams.map(([team, rating], index) => {
        ...
        <tr>
            <td>${index + 1}</td>
            <td class="team-name-cell">${displayName}</td>
            <td class="rating-cell">${Math.round(rating)}</td>
        </tr>
    `}).join('');
