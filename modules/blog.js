...
function renderArticleList(page) {
    ...
    const currentPosts = window.blogPosts.slice(start, start + articlesPerPage);
    currentPosts.forEach(post => {
        ...
        card.onclick = () => window.showPage.showArticleDetail(post.slug, post.title);
...
function renderPagination() {
    ...
    const totalPages = Math.ceil(window.blogPosts.length / articlesPerPage);
...
function showBlogList() {
    window.isShowingArticleDetail = false;
    const listContainer = document.getElementById('blog-list-container');
    const paginationContainer = document.getElementById('pagination');
...
async function renderArticleDetail(slug, title, fromPopState = false) {
    window.isShowingArticleDetail = true;
    const listContainer = document.getElementById('blog-list-container');
    const paginationContainer = document.getElementById('pagination');
    const contentDiv = document.getElementById('blog-content');
